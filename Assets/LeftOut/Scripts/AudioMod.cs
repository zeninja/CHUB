...
public class AudioMod : MonoBehaviour
{
    ...
    {        
        // Debug.Log("STARTING SLIDER AUDIO. " + MetaSlider.GetInstance().stageInfo.level);


        if (targetStage.world == MetaSlider.GetInstance().stageInfo.world &&
            targetStage.level == MetaSlider.GetInstance().stageInfo.level)
        {
            string targetSound = "Hallway " + targetStage.world + "-" + targetStage.level;
            AudioManager.GetInstance().Play(targetSound);
